...
/* Explanation: 
company: works because it is declared with var, which is function-scoped.
designation: doesn't work because it is declared with let, which is block-scoped.
salary" doesn't work because it is declared with const, which is also block-scoped.
The program stops after the designation error, so the salary line is not executed.*/